...
static bool wait_for(const std::function<bool()> &listo, int msMax = 3000)
{
    QElapsedTimer clock;
    clock.start();
    while (!listo() && clock.elapsed() < msMax) {
...
// used to break.
static QByteArray speak(const QList<QByteArray> &trozos, int pausaMs = 150)
{
    QTcpSocket socket;
...
int main(int argc, char **argv)
{
    ...
    settings.lanToken = QStringLiteral("testigo-de-test");
    settings.lanAllowRing = true;

    LanServer servidor;
    servidor.setStatusProvider([] { return QByteArray("{\"ringing\":false}"); });
    QObject::connect(&servidor, &LanServer::command, [](const QString &verbo) {
        ++ordenes;
        ultimaOrden = verbo;
    });
    servidor.configure(settings);

    check("the server listens", wait_for([&] { return servidor.listening(); }));
    ...
    {
        const QByteArray cuerpo = "Authorization: Bearer testigo-de-test";
        const QByteArray r = speak({"POST /orden HTTP/1.1\r\nHost: x\r\nContent-Length: "
    ...
    std::printf("\n--- a request split across two TCP segments ---\n");
    {
        ...
        const QByteArray r = speak({"GET /state HTTP/1.1\r\nHost: x\r\nAuthor",
                                     "ization: Bearer testigo-de-test\r\n\r\n"});
        check("the split headers are reassembled", r.startsWith("HTTP/1.1 200"));
        check("and it answers the status", r.contains("ringing"));
        check("without firing any order", ordenes == antes);
    }

    std::printf("\n--- a POST with the body in another segment ---\n");
    {
        const int antes = ordenes;
        speak({"POST /orden HTTP/1.1\r\nHost: x\r\nAuthorization: Bearer testigo-de-test\r\n"
                "Content-Length: 5\r\n\r\n",
                "sonar"});
        check("the order arrives even when the body comes separately",
                  wait_for([&] { return ordenes == antes + 1; }));
        check("and it arrives WHOLE, not cut off", ultimaOrden == QLatin1String("sonar"));
    }
    ...
    std::printf("\n--- a normal request, in one piece ---\n");
    {
        ...
        check("it fires the order", wait_for([&] { return ordenes == antes + 1; }));
        check("and it is the one that was sent", ultimaOrden == QLatin1String("parar"));
    }
    ...
    std::printf("\n--- the simple door, off ---\n");
    {
        ...
        // does not open.
        const int antes = ordenes;
        const QByteArray r = speak({"GET /sonar?key=abreteSesamo HTTP/1.1\r\nHost: x\r\n\r\n"});
        check("off, not even with the good key", r.startsWith("HTTP/1.1 401"));
        check("and it fires no order", ordenes == antes);
    }

    settings.lanSimpleEnabled = true;
    settings.lanSimpleKey = QStringLiteral("abreteSesamo");
    servidor.configure(settings);
    ...
    std::printf("\n--- the simple door, on ---\n");
    {
        ...
        check("with the wrong key, 401",
                  speak({"GET /sonar?key=otra HTTP/1.1\r\nHost: x\r\n\r\n"})
                      .startsWith("HTTP/1.1 401"));
        ...
        check("a prefix of the key is no good",
                  speak({"GET /sonar?key=abre HTTP/1.1\r\nHost: x\r\n\r\n"})
                      .startsWith("HTTP/1.1 401"));
        check("none of that fired an order", ordenes == antes);

        const QByteArray r =
            speak({"GET /sonar?key=abreteSesamo HTTP/1.1\r\nHost: x\r\n\r\n"});
        check("with the good key, 200", r.startsWith("HTTP/1.1 200"));
        check("and the order arrives", wait_for([&] { return ordenes == antes + 1; }));
        check("and it is the one that was asked for", ultimaOrden == QLatin1String("sonar"));
    ...
    }
    {
        ...
        const int antes = ordenes;
        for (const char *path : {"GET /state?key=abreteSesamo HTTP/1.1\r\nHost: x\r\n\r\n",
                                 "GET /donde?key=abreteSesamo HTTP/1.1\r\nHost: x\r\n\r\n",
                                 "GET /bloquear?key=abreteSesamo HTTP/1.1\r\nHost: x\r\n\r\n"}) {
            const QByteArray r = speak({QByteArray(path)});
            check("the simple door rejects it", r.startsWith("HTTP/1.1 403"));
        }
        check("and none of the three fired anything", ordenes == antes);

        // Stop, yes, which is the other half of a home-automation button.
        const QByteArray r =
            speak({"GET /parar?key=abreteSesamo HTTP/1.1\r\nHost: x\r\n\r\n"});
        check("but stop, yes", r.startsWith("HTTP/1.1 200"));
        check("and it arrives", wait_for([&] { return ordenes == antes + 1; }));
    }
    {
