...
int main(int argc, char **argv)
{
    ...
    wipeConfig();
    {
        ...
        check("no SMS key", s.smsKey.isEmpty());
        ...
        // one.
        check("locate by SMS comes on", s.smsAllowLocate == true);
        check("but without a key it obeys nobody", s.smsKey.isEmpty());
        check("the relay comes off", s.relayEnabled == false);
        ...
        check("power off the phone comes off", s.relayAllowPower == false);
        check("the home channel comes off", s.lanEnabled == false);
        check("ntfy comes off", s.ntfyEnabled == false);
        ...
        check("the default melody is Blip", s.ringSound.endsWith(QLatin1String("Blip.oga")));
        check("at full volume by default", s.ringVolume == 100);
        check("and it rings for two minutes", s.ringSeconds == 120);
        check("lock by SMS comes off", s.smsAllowLock == false);
        check("no last position", s.lastKnown.valid == false);

        // What really matters about a fresh install: it obeys nobody.
        check("freshly installed it will not even ring",
    ...
    std::printf("\n--- the capability matrix ---\n");
    {
        ...
        s.smsKey.clear();
        check("without a key it does not ring", !LostPhoneConfig::allows(s, Channel::Sms, Capability::Ring));
        check("without a key it does not locate",
                  !LostPhoneConfig::allows(s, Channel::Sms, Capability::Locate));

        s.smsKey = QStringLiteral("abcd1234");
        check("with key and permission, it rings",
                  LostPhoneConfig::allows(s, Channel::Sms, Capability::Ring));
        check("with key and permission, it locates",
                  LostPhoneConfig::allows(s, Channel::Sms, Capability::Locate));
        check("lock stays forbidden if it was not turned on",
        ...
        s.enabled = false;
        check("the master switch turns everything off",
                  !LostPhoneConfig::allows(s, Channel::Sms, Capability::Ring));

        // Each channel has its own credential and permissions, and they lend
        // nothing to one another.
        s.enabled = true;
        check("the relay without a witness obeys nobody",
                  !LostPhoneConfig::allows(s, Channel::Relay, Capability::Ring));
        check("the home channel without a witness obeys nobody",
        ...
        check("and powering off the phone too",
                  !LostPhoneConfig::allows(s, Channel::Relay, Capability::Power));
        s.relayAllowPower = true;
        check("turned on by hand, the panel can indeed power it off",
                  LostPhoneConfig::allows(s, Channel::Relay, Capability::Power));
        ...
        check("ntfy with a key but no topic does not obey",
                  !LostPhoneConfig::allows(s, Channel::Ntfy, Capability::Ring));
        s.ntfyTopic = QStringLiteral("lost-phone-abcdefgh");
        check("with topic and key, ntfy obeys",
                  LostPhoneConfig::allows(s, Channel::Ntfy, Capability::Ring));
        check("and locate over ntfy comes off",
        ...
        check("the SMS key is not the ntfy one", s.smsKey != s.ntfyKey);
        check("the relay witness is not the home one", s.relayToken != s.lanToken);

        // And the master switch still rules over all four.
        s.enabled = false;
        check("the master also turns off the relay",
                  !LostPhoneConfig::allows(s, Channel::Relay, Capability::Ring));
        check("the master also turns off the home channel",
                  !LostPhoneConfig::allows(s, Channel::Lan, Capability::Ring));
        check("the master also turns off ntfy",
                  !LostPhoneConfig::allows(s, Channel::Ntfy, Capability::Ring));
    }
    ...
    }
    {
        ...
        s.ringVolume = 400;
        LostPhoneConfig::save(s);
        check("an absurd volume is clamped to 100", LostPhoneConfig::load().ringVolume == 100);
        s.ringVolume = -5;
        LostPhoneConfig::save(s);
        check("and a negative one does not leave the alarm mute",
    ...
    wipeConfig();
    {
        ...
        const Settings leida = LostPhoneConfig::load();
        check("enabled survives", leida.enabled == true);
        check("the key survives", leida.smsKey == QLatin1String("perrogato"));
        check("the locate permission survives", leida.smsAllowLocate == true);
        check("the duration survives", leida.ringSeconds == 45);
        check("turning off the GPS survives", leida.locateGnss == false);
    }
    ...
    std::printf("\n--- only the daemon writes the last position ---\n");
    {
        ...
        const Settings leida = LostPhoneConfig::load();
        check("the position is still there after saving settings", leida.lastKnown.valid == true);
        check("and with its coordinates intact",
